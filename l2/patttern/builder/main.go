...
func (d *Director) Build(name, surname, gender string, age int) *Person {
	d.b.SetName(name)
	d.b.SetSurname(surname)
	d.b.SetGender(gender)
	d.b.SetAge(age)
...
func (c *ConcreteBuilder) SetGender(gender string) {
	c.p.Gender = gender
}
...
type Person struct {
	Name, Surname, Gender string
	Age                   int
}

func main() {
	d := NewDirector()
	fmt.Println(d.Build("Gata", "Kamsky", "Man", 55))
}
